import { useEffect, useState } from 'react';
import type { PluginCatalogAdapter } from '../adapters/plugin-catalog-adapter';
import type { ValidationRuntimeAdapter } from '../adapters/validation-runtime-adapter';
import { useProject } from '../context/ProjectContext';
import { useWorkflowStore } from '../store/workflow-store';
import { ProjectManagerModal } from './ProjectManagerModal';
import { Button } from './ui/button';

export interface WorkflowHeaderProps {
  catalog?: PluginCatalogAdapter;
  validation?: ValidationRuntimeAdapter;
  onSave?: () => Promise<void>;
}

export function WorkflowHeader({ onSave }: WorkflowHeaderProps) {
  const { currentProject, isDirty, saveCurrentProject, isLoading } = useProject();
  const workflow = useWorkflowStore((s) => s.workflow);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [savedFeedback, setSavedFeedback] = useState(false);

  // Captura de atalho Ctrl+S / Cmd+S (RF-02, D-07)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
        e.preventDefault();
        if (onSave) {
          onSave().then(() => {
            setSavedFeedback(true);
            setTimeout(() => setSavedFeedback(false), 2000);
          });
        } else if (currentProject) {
          saveCurrentProject().then(() => {
            setSavedFeedback(true);
            setTimeout(() => setSavedFeedback(false), 2000);
          });
        } else {
          setIsModalOpen(true);
        }
      }
    };

    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [currentProject, saveCurrentProject, onSave]);

  const handleManualSave = async () => {
    if (onSave) {
      await onSave();
      setSavedFeedback(true);
      setTimeout(() => setSavedFeedback(false), 2000);
    } else if (currentProject) {
      await saveCurrentProject();
      setSavedFeedback(true);
      setTimeout(() => setSavedFeedback(false), 2000);
    } else {
      setIsModalOpen(true);
    }
  };

  const displayName = currentProject?.name || workflow.name || 'Sem Projeto';

  return (
    <>
      <header className="z-20 flex h-[62px] shrink-0 items-center gap-4 border-b border-slate-200 bg-white/95 px-5 shadow-sm backdrop-blur-xl">
        <div className="flex items-center gap-2.5 text-[15px] font-bold tracking-tight">
          <span className="grid h-8 w-8 place-items-center rounded-[10px] bg-gradient-to-br from-indigo-500 to-indigo-700 text-white shadow-lg shadow-indigo-200">
            R
          </span>
          <span>RunFlux</span>
        </div>

        <span className="h-6 w-px bg-slate-200" />

        {/* Informações do Projeto e Botão do Modal */}
        <div className="flex items-center gap-3">
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-1.5 text-left hover:border-indigo-300 hover:bg-slate-50 transition-all"
            title="Abrir Gerenciador de Projetos"
            data-testid="project-selector-btn"
          >
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <span
                  data-testid="current-project-name"
                  className="font-semibold text-xs text-slate-800 max-w-[200px] truncate"
                >
                  {displayName}
                </span>
                {currentProject?.currentWorkflowVersion && (
                  <span className="rounded bg-slate-100 px-1 py-0.2 text-[10px] font-mono text-slate-500">
                    {currentProject.currentWorkflowVersion}
                  </span>
                )}
              </div>
              <span className="text-[10px] text-slate-400">
                {workflow.nodes.length} nós · {workflow.connections.length} conexões
              </span>
            </div>
            <span className="text-xs text-slate-400">▼</span>
          </button>

          {/* Badge de status: Salvo vs Não salvo */}
          {isDirty ? (
            <span
              data-testid="dirty-badge"
              className="flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-0.5 text-[10px] font-semibold text-amber-700 border border-amber-200"
            >
              <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-pulse" />
              Alterações não salvas
            </span>
          ) : savedFeedback ? (
            <span className="text-[11px] font-semibold text-emerald-600">
              ✓ Salvo
            </span>
          ) : (
            <span className="text-[10px] text-slate-400">Salvo</span>
          )}
        </div>

        {/* Ações à direita */}
        <div className="ml-auto flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={handleManualSave}
            disabled={isLoading || (!isDirty && !savedFeedback && !!currentProject)}
            className="text-xs font-medium"
            data-testid="save-project-btn"
          >
            {isLoading ? 'Salvando...' : 'Salvar (Ctrl+S)'}
          </Button>

          <Button
            variant="outline"
            size="sm"
            onClick={() => setIsModalOpen(true)}
            className="text-xs font-medium"
            data-testid="open-projects-modal-btn"
          >
            📁 Projetos
          </Button>
        </div>
      </header>

      <ProjectManagerModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
      />
    </>
  );
}
