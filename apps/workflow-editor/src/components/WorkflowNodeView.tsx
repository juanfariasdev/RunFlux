...
export function WorkflowNodeView({ id, data, selected, dragging }: NodeProps<FlowNode>) {
  ...
  const color = appearance.color ?? categoryColor(category, data?.manifest?.appearance?.accent);
    ...
    <>
      ...
      >
        ...
        <div className={`flex h-full min-h-20 items-center ${isDiamond ? 'justify-center px-[25%] py-4 text-center' : 'gap-3 px-4 py-3'}`}>
          {!isDiamond && <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-indigo-50 text-[var(--node-accent)]"><svg className="h-[19px] w-[19px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><CategoryIcon category={category} icon={data?.manifest?.appearance?.icon} /></svg></span>}
...
/** Colors for the accents a manifest may name (`appearance.accent`). */
const ACCENT_COLORS: Readonly<Record<string, string>> = { cyan: '#0891b2', violet: '#8b5cf6' };

function categoryColor(category: string | undefined, accent?: string) {
  if (accent && Object.hasOwn(ACCENT_COLORS, accent)) return ACCENT_COLORS[accent];
...
function CategoryIcon({ category, icon }: { category: string | undefined; icon?: string }) {
  if (icon === 'database') return <><ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" /><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" /></>;
  if (icon === 'code') return <path d="m16 18 6-6-6-6M8 6l-6 6 6 6" />;
